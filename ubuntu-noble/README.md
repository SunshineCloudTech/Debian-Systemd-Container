# Ubuntu Noble (24.04 LTS) Systemd Container

## 概述

这是基于 Ubuntu 24.04 LTS (Noble Numbat) 的 systemd 容器开发环境。提供完整的多语言开发工具链和 systemd 初始化系统支持。

## 基础镜像

- 基础: `buildpack-deps:noble`
...
# 进入容器
docker compose -f docker-compose.ubuntu-noble.yml exec ubuntu-noble bash
```
...
# 运行容器
docker run -d --name ubuntu-noble \
  --privileged \
  -v /sys/fs/cgroup:/sys/fs/cgroup:rw \
  --cgroupns=host \
  sunshinecloud007/sunshinecloud-systemd-devcontainer:ubuntu-noble
```
...
- `ubuntu-noble-base`: 构建镜像（未压缩）
- `ubuntu-noble`: 生产镜像（压缩优化）
- `ubuntu-noble-latest`: 最新版本别名

## 版本特性

Ubuntu 24.04 LTS (Noble Numbat) 是最新的长期支持版本：
...
- Ubuntu Noble 发布说明: https://releases.ubuntu.com/24.04/
