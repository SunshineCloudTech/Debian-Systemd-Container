# Ubuntu Jammy (22.04 LTS) Systemd Container

## 概述

这是基于 Ubuntu 22.04 LTS (Jammy Jellyfish) 的 systemd 容器开发环境。提供完整的多语言开发工具链和 systemd 初始化系统支持。

## 基础镜像

- 基础: `buildpack-deps:jammy`
...
# 进入容器
docker compose -f docker-compose.ubuntu-jammy.yml exec ubuntu-jammy bash
```
...
```bash
# 拉取镜像（多架构，自动选择对应架构）
docker pull sunshinecloud007/sunshinecloud-systemd-devcontainer:ubuntu-jammy

# 运行容器
docker run -d --name ubuntu-jammy \
  --privileged \
  -v /sys/fs/cgroup:/sys/fs/cgroup:rw \
  --cgroupns=host \
  sunshinecloud007/sunshinecloud-systemd-devcontainer:ubuntu-jammy
```
...
- `ubuntu-jammy-base`: 构建镜像（未压缩）
- `ubuntu-jammy`: 生产镜像（压缩优化）
- `ubuntu-jammy-latest`: 最新版本别名

## 版本特性

Ubuntu 22.04 LTS (Jammy Jellyfish) 是广泛使用的长期支持版本：
...
- Docker Hub: https://hub.docker.com/r/sunshinecloud007/sunshinecloud-systemd-devcontainer
- GitHub: https://github.com/SunshineCloudTech/Debian-Systemd-Container
- Ubuntu Jammy 发布说明: https://releases.ubuntu.com/22.04/
