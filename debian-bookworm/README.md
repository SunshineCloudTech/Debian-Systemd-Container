# Debian Bookworm Systemd Container

## 概述

这是基于 Debian 12 (Bookworm) 的 systemd 容器开发环境。提供完整的多语言开发工具链和 systemd 初始化系统支持。

## 基础镜像

- 基础: `buildpack-deps:bookworm`
...
# 进入容器
docker compose -f docker-compose.debian-bookworm.yml exec debian-bookworm bash
```
...
# 运行容器
docker run -d --name debian-bookworm \
  --privileged \
...
- `debian-bookworm-base`: 构建镜像（未压缩）
- `debian-bookworm`: 生产镜像（压缩优化）
- `debian-bookworm-latest`: 最新版本别名
